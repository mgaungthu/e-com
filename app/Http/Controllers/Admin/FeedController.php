...
class FeedController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $request->validate([
            ...
            'status' => ['nullable', Rule::enum(FeedStatus::class)],
        ...
        $feeds = Feed::query()
            ...
            ->withCount([
                'likes',
                'comments' => fn ($query) => $query->where('is_active', true),
            ])
            ->when(
    ...
    public function store(StoreFeedRequest $request): JsonResponse
    {
        ...
        $feed = DB::transaction(function () use ($request, $data): Feed {
            ...
            if (
                ! $publishedAt
                && $data['status'] === FeedStatus::Published->value
            ) {
                $publishedAt = now();
            }
    ...
    public function update(
        ...
        DB::transaction(function () use (
            ...
            /*
             * If published_at was explicitly sent,
            ...
            /*
             * If the feed is being published for the first time
             * and no published_at was provided, publish it now.
             */
            if (
                $data['status'] === FeedStatus::Published->value
                && ! $feed->published_at
    ...
    private function syncMedia(
        ...
        $order = collect(
            ...
            ->map(fn ($id) => (int) $id)
            ->filter(
                fn ($id) => $remaining->has($id)
            );

        $ordered = $order
            ->map(
                fn ($id) => $remaining->get($id)
            )
            ->concat(
                $remaining
                    ->except($order->all())
